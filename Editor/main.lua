...
function main()
    ...
    register_platform_callback( 'CHILD_VIEW_LBUTTON_DOWN', function( args )
        ...
        local alt_flag = ( down_args[5] == 'true' )

        down_flag = true

        if shift_flag then                       -- 旋转
            if g_edit_mb and g_edit_node then
        ...
        elseif alt_flag then                  -- 平移大地图
            sm_move_flag = true
        elseif ctrl_flag then
            if g_edit_mb and g_edit_node then
                copy_flag = true
                g_edit_node = g_edit_mb:copyObject( g_edit_node )
            end
        else                                    -- 选择
    ...
    register_platform_callback( 'CHILD_VIEW_MOUSE_MOVE', function( args )
        ...
        CCLuaLog( 'alt_flag : ' .. tostring( alt_flag ) )
        CCLuaLog( 'offset_y : ' .. tostring( offset_y ) )
        ...
        if shift_flag then
            if rotation_flag then
                ...
                if m_x then
                    ...
                    local angle = math.acos( dot_value / dis ) * 180.0 / math.pi;
        ...
        elseif alt_flag then
            if sm_move_flag and g_sm_node then
                local x = g_sm_node:getPositionX() + offset_x
                local y = g_sm_node:getPositionY() - offset_y
                g_sm_node:setPosition( x, y )
            end
        elseif ctrl_flag then
            if copy_flag and g_edit_mb and g_edit_node then
                g_edit_mb:moveObject( g_edit_node, offset_x, -offset_y )
            end
        else
